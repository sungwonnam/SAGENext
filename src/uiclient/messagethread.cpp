...
MessageThread::MessageThread(int sock, const quint64 uiid, QObject *parent) :
	QThread(parent), sockfd(sock), uiclientid(uiid)
{
	end = false;
}
...
//		qDebug("MessageThread::%s() : new message arrived [%s]",__FUNCTION__, msg.constData());
		...
		case ACK_FROM_WALL : {
				...
				qDebug("MessageThread::%s() : received ACK [%s]", __FUNCTION__, msg.constData());
